...
Milestone 4C private-media addendum: a typed frontend catalogue maps local ignored Red Masai owner-demo media into the homepage, Stay, Experiences, Celebrate, Create, Gallery, Our Story and Location. Only literal `NEXT_PUBLIC_RED_MASAI_DEMO_MEDIA=true` enables it; false, missing, malformed or failed assets use polished fixed-ratio placeholders without requesting local URLs. Video is muted, controlled, inline and non-autoplaying. No local media or derivative may enter Git history or a public deployment. The public repository must continue to build with the media directory absent.
